...
<div class="card">
    ...
    <div class="card-body">
        <form class="form-material mt-4" action="{{ route("admin.kabupaten.update", $val->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('put')
            <div class="form-group {{ $errors->has('id_prov') ? 'has-error' : '' }}">
                ...
                <select name="id_prov" id="id_prov" class="form-control select2" required style="width: 100%; height:36px;">
                    ...
                    @foreach($prov as $id => $row)
                        <option value="{{ $id }}" @if($val->id_prov == $id) selected @endif name="{{ $row }}">{{ $row }}</option>
                    @endforeach
                </select>
            ...
            <div class="row">
                <div class="col">
                    <div class="form-group {{ $errors->has('id_kab') ? 'has-error' : '' }}">
                        ...
                        <input type="text" id="id_kab" name="id_kab" class="form-control" value="{{ $val->id_kab }}">
